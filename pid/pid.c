...
void pid_init(pid_data_t *pid, float p_factor, float i_factor, float d_factor) {
    ...
    pid->max_error = ((int32_t)INT16_MAX << PID_FBITS) / (pid->p_factor + 1);
...
int16_t pid_update(pid_data_t *pid, int16_t target, int16_t measured) {
    ...
    if (error > pid->max_error) {
        ...
        p = ((int32_t)pid->p_factor * (int32_t)error) >> PID_FBITS;
    }
    ...
    if (sum > pid->max_sum_error) {
        ...
        pid->sum_error = sum;
        i = ((int32_t)pid->i_factor * (int32_t)pid->sum_error) >> PID_FBITS;
    }

    // Compute derivative
    d = ((int32_t)pid->d_factor * ((int32_t)pid->last_measured - measured)) >> PID_FBITS;
    pid->last_measured = measured;

    int32_t result = (p + i + d);
